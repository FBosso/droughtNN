...
def global_timeseries_from_folder_full(path,startyr,endyr,lead=1, temp_res='monthly'):
    ...
    if temp_res == 'monthly':
        ...
        for file in files:
            if (int(file.split('-')[0]) >= startyr) and (int(file.split('-')[0]) <= endyr):
                # take each of the considered files and append the daily mean to the avg_item list
                items.append(xr.open_dataset(f'{path}/{file}', engine='netcdf4'))
        # concatenate the daily means on the time dimension
        ...
        #replace NaN values with -99999 (otherwise rolling does not work)
...
def generate_full_dataset(startyr,endyr,combo,temp_res='monthly',lead=1, month_label = False):
    ...
    temp_res : TYPE string ['monthly' or 'moving_monthly_avg']
        DESCRIPTION. string that indicate if the temporal resolution of the dataset has to be moving monthly average of monthly
