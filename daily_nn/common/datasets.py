...
# -*- coding: utf-8 -*-
"""
Dataset construction and global-field processing helpers for the daily_nn
pipeline (local timeseries, global gridded fields, PCA reduction, splits).
"""
import os
import random
...
from . import config
from .combo_utils import detect_global
...
def generate_full_dataset(startyr, endyr, combo, temp_res='monthly', lead=1, month_label=False):
    ...
    if temp_res == 'monthly':
        ...
        target = timeseries_from_folder_full(str(config.LOCAL_DATA_DAILY_DIR / 'tp'), startyr, endyr, target=True, temp_res=temp_res)
